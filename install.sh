...
# ~/.cidx; no machine-specific path is forced (only CIDX_HOME is
...
# os.homedir() already returns the real ~ and the daemon uses ~/.cidx.
# Only if the user EXPLICITLY provides CIDX_HOME do we pass it to the service/CLI.
if [ -n "${CIDX_HOME:-}" ]; then
  HOME_OVERRIDE="$CIDX_HOME"
  DATA_HOME="$CIDX_HOME"
else
  HOME_OVERRIDE=""
  DATA_HOME="$HOME/.cidx"   # for informational messages only
fi

printf "\n%scidx installation%s\n" "$BOLD" "$NC"
printf "repo: %s\n\n" "$REPO_DIR"
...
make_wrapper() {
  ...
  [ -n "$HOME_OVERRIDE" ] && home_line="export CIDX_HOME=\"\${CIDX_HOME:-$HOME_OVERRIDE}\""
  cat > "$BIN_DIR/$name" <<EOF
#!/usr/bin/env bash
# cidx CLI wrapper (generated by install.sh)
$home_line
...
# Legacy cleanup: installs from before the cidx rename used a differently named
# unit. Stop and remove it so only one daemon service remains.
LEGACY_SERVICE="mcp-code-indexer"
if command -v systemctl >/dev/null 2>&1 && [ -d /run/systemd/system ]; then
  if systemctl --user list-unit-files 2>/dev/null | grep -q "^${LEGACY_SERVICE}\.service"; then
    info "Removing legacy systemd unit: ${LEGACY_SERVICE}.service (renamed to ${SERVICE_NAME})"
...
setup_systemd() {
  ...
  # otherwise the daemon uses the natural ~/.cidx (no machine-specific path forced).
  local home_env=""
  [ -n "$HOME_OVERRIDE" ] && home_env="Environment=CIDX_HOME=$HOME_OVERRIDE"
  cat > "$unit" <<EOF
[Unit]
Description=cidx daemon
